                ...
                <div class="row">
                    ...
                    <div class="col-lg-4">
                        <form action="{{ route('transaksi.simpan') }}" class="form-penjualan" method="post">
                            ...
                            <input type="hidden" name="total" id="total" value="{{ isset($penjualan['total_harga']) ? $penjualan['total_harga'] : '' }}">
...
<script>
    ...
    function loadForm(diskon = 0, diterima = 0) {
        let penjualan_total = {{ isset($penjualan['total_harga']) ? $penjualan['total_harga'] : 'null' }};
        // let total_text = $('.total').text();
        let total_text = penjualan_total != 0 ? penjualan_total : $('.total').text();
        console.log(total_text);
        // $('#total').val($('.total').text());
        $('#total').val(total_text);
        $('#total_item').val($('.total_item').text());

        // $.get(`{{ url('/transaksi/loadform') }}/${diskon}/${$('.total').text()}/${diterima}`)
        $.get(`{{ url('/transaksi/loadform') }}/${diskon}/${total_text}/${diterima}`)
            .done(response => {
